import ElementBuilder from '../modules/element-builder.mjs';
import { relativeTime } from '../modules/date-utils.mjs';

const keywordContainer = document.getElementById('keyword-container');
const noteCardContainer = document.getElementById('note-card-container');

const openFilter = document.getElementById('open-filter');
const clearFilter = document.getElementById('clear-filter');

import { dataConstraints } from "../../../config.js";

const MIN_RATING_COUNT_TO_SHOW = dataConstraints.MIN_RATING_COUNT_TO_SHOW;

let notes = [];
let keywordCount = new Map();
let keywordFilter = [];

const sortByInput = document.getElementById('sort-by-input');
const sortDirectionInput = document.getElementById('sort-direction-input');
let sortNotesBy = 'views';
let sortNotesAscending = false;

openFilter.addEventListener('click', () => {
	if(keywordContainer.classList.contains('open')) keywordContainer.classList.remove('open');
	else keywordContainer.classList.add('open');
});

clearFilter.addEventListener('click', () => {
	const filterChips = document.getElementsByClassName('keyword-filter-input');

	for(const filterChip of filterChips) {
		filterChip.checked = false;
	}

	keywordFilter = [];

	loadNotes();
});

const sortContainer = document.getElementById('sort-container');
const openSort = document.getElementById('open-sort');

openSort.addEventListener('click', () => {
	if(sortContainer.classList.contains('open')) sortContainer.classList.remove('open');
	else sortContainer.classList.add('open');
});

await fetchNotes();
loadNotes();

async function loadNotes() {
	noteCardContainer.innerHTML = '';

	let filteredNotes = notes;

	if(keywordFilter.length > 0) {
		filteredNotes = notes.filter(note => checkSubset(keywordFilter, note.keywords));
	}

	for(let i = 0; i < filteredNotes.length; i++) {
		const note = filteredNotes[i];
		loadNote(note);
	}
}

sortByInput.addEventListener('input', sortNotes);
sortDirectionInput.addEventListener('input', sortNotes);

function sortNotes() {
	sortNotesBy = sortByInput.value;
	sortNotesAscending = (sortDirectionInput.value === 'ascending');

	console.table(sortNotesBy, sortNotesAscending);

	let sortFunction = () => {};

	switch (sortNotesBy) {
		case 'views':
		case 'rating':
			sortFunction = (a, b) => a[sortNotesBy] - b[sortNotesBy];
			break;
		case 'created-at':
		case 'updated-at':
			sortFunction = (a, b) => new Date(a[sortNotesBy]).getTime() - new Date(b[sortNotesBy]).getTime();
			break;
		case 'trending':
			sortFunction = (a, b) => {
				a = a.views / (((Date.now() - new Date(a.createdAt).getTime()) / 1_000 + 2) / 86_400) ** 2;
				b = b.views / (((Date.now() - new Date(b.createdAt).getTime()) / 1_000 + 2) / 86_400) ** 2;
				return a - b;
			}
	}

	notes.sort(sortFunction);

	if (!sortNotesAscending) notes.reverse();

	console.table(notes.map(note => ({ title: note.title, updatedAt: new Date(note.updatedAt).getTime() })));

	loadNotes();
}

function checkIntersection(arr1, arr2) {
	return arr1.some(item => arr2.includes(item));
}

/**
 * Check if an array is a subset of another array.
 *
 * @param {array} superset - The superset
 * @param {array} subset - The subset
 */
function checkSubset(superset, subset) {
	return superset.every(item => subset.includes(item));
}

function addKeyword(keyword, count) {
	const keywordWrapper = new ElementBuilder('label')
		.classes([ 'chip', 'keyword-filter' ])
		.appendTo(keywordContainer);

	const checkbox = new ElementBuilder('input')
		.classes([ 'keyword-filter-input' ])
		.attributes({
			type: 'checkbox',
			hidden: true,
			value: keyword
		})
		.appendTo(keywordWrapper);

	checkbox.addEventListener('change', () => {
		if(checkbox.checked) {
			keywordFilter.push(checkbox.value);
		} else {
			keywordFilter.splice(keywordFilter.indexOf(checkbox.value), 1)
		}

		loadNotes();
	});

	new ElementBuilder('span')
		.text(keyword)
		.appendTo(keywordWrapper);

	new ElementBuilder('span')
		.text(count)
		.classes([ 'keyword-count' ])
		.appendTo(keywordWrapper);
}

async function loadNote(note) {
	const containerType = (window.innerWidth > 1000) ? 'note-card' : 'note-card-small';
	const noteCard = new ElementBuilder('div')
		.classes([ containerType ])
		.appendTo(noteCardContainer);

	new ElementBuilder('a')
		.classes([ 'link' ])
		.attributes({
			href: `/note/${note.id}`
		})
		.appendTo(noteCard);

	if(note.thumbnailURL) {
		new ElementBuilder('div')
			.innerHTML(`<img src="${note.thumbnailURL}" />`)
			.classes([ 'thumbnail' ])
			.appendTo(noteCard);
	}

	const metadata = new ElementBuilder('div')
		.classes([ 'meta-data' ])
		.appendTo(noteCard);

	new ElementBuilder('div')
		.text(note.title)
		.classes([ 'title' ])
		.appendTo(metadata);

	new ElementBuilder('div')
		.text(note.authorDisplayName)
		.classes([ 'author' ])
		.appendTo(metadata);

	const details = new ElementBuilder('div')
		.classes([ 'details' ])
		.appendTo(metadata);

	new ElementBuilder('span')
		.text(relativeTime(new Date(note.createdAt)))
		.classes([ 'date' ])
		.appendTo(details);

	new ElementBuilder('span')
		.text(`${note.views} views`)
		.classes([ 'views' ])
		.appendTo(details);

	if(note.rateCount >= MIN_RATING_COUNT_TO_SHOW) {
		new ElementBuilder('span')
			.innerHTML(`${note.views} <span class="material-symbols-outlined">star</span>`)
			.classes([ 'rating' ])
			.appendTo(details);
	}

	if(note.keywords.length > 0) {
		const keywordContainer = new ElementBuilder('div')
			.classes([ 'keyword-container' ])
			.appendTo(metadata);

		for(let i = 0; i < note.keywords.length; i++) {
			new ElementBuilder('label')
				.innerHTML(`<a href="${note.keywords[i]}">${note.keywords[i]}</a>`)
				.classes([ 'chip', 'keyword' ])
				.appendTo(keywordContainer);
		}
	}
}

async function fetchNotes() {
	try {
		const response = await fetch('/api/notes');

		const json = await response.json();

		if(!response.ok) {
			console.log(json.error);
			return;
		}

		notes = json.data;
	} catch (error) {
		console.error(error);
	}

	for(let i = 0; i < notes.length; i++) {
		const note = notes[i];

		for(let j = 0; j < note.keywords.length; j++) {
			const keyword = note.keywords[j];

			if(keywordCount.get(keyword)) {
				const count = keywordCount.get(keyword);
				keywordCount.set(keyword, count + 1);
			} else {
				keywordCount.set(keyword, 1)
			}
		}
	}

	// Sort keys alphabetically.
	keywordCount = new Map([...keywordCount].sort((a, b) => {
		if(a[0] > b[0]) return 1;
		else if(a[0] < b[0]) return -1;
		return 0;
	}));

	keywordCount.forEach((value, key) => addKeyword(key, value));
}